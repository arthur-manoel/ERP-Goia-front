              ...
              <div className="max-h-72 overflow-y-auto">
                {!user?.empresaId ? <div className="p-4 text-xs text-muted-foreground">Selecione uma empresa para visualizar alertas.</div>
                  ...
                  : stockAlerts.map(item => <button key={item.id} onClick={() => router.push('/alertas-estoque')} className="w-full px-4 py-3 text-left border-b border-border/60 hover:bg-secondary/50 flex gap-3">
                      ...
                      <span className="min-w-0"><span className="block text-sm font-medium truncate">{item.produto}</span><span className="block text-[11px] text-muted-foreground">{item.estoque} · disponível {Number(item.disponivel || 0).toLocaleString('pt-BR', { maximumFractionDigits: 2 })} / mínimo {Number(item.minimo || 0).toLocaleString('pt-BR', { maximumFractionDigits: 2 })}</span></span>
                    </button>)}
              </div>
            </PopoverContent>
          </Popover>
