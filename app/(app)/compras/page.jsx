...
export default function ComprasPage(){
 ...
 const remove=async row=>{const r=await fetch(`/api/compras/${row.id}?empresaId=${user.empresaId}`,{method:'DELETE'});if(!r.ok){const b=await r.json();return toast.error(b.error)}toast.success('Compra excluída.');load()}
 const columns=[
  ...
  {key:'status',label:'Status',render:r=><StatusBadge status={r.status}/>},{key:'actions',label:'',cellClass:'text-right',render:r=><div className="flex justify-end gap-1"><Button asChild size="icon" variant="ghost"><Link href={`/compras/${r.id}`} title="Abrir compra"><Eye className="h-4 w-4"/></Link></Button>{r.origem==='MANUAL'&&r.status!=='entregue'&&<AlertDialog><AlertDialogTrigger asChild><Button size="icon" variant="ghost" className="text-red-400"><Trash2 className="h-4 w-4"/></Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Excluir a compra {r.codigo}?</AlertDialogTitle><AlertDialogDescription>A compra e seus produtos serão excluídos permanentemente.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancelar</AlertDialogCancel><AlertDialogAction onClick={()=>remove(r)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Excluir compra</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>}</div>}
 ]
