...
export default function NovaOPPage() {
  ...
  return <div><PageHeader title="Nova ordem de produção" description="Etapa 1 de 3 — dados gerais" icon={Factory}/><Card className="max-w-5xl"><CardContent className="p-6"><form onSubmit={save} className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
    ...
    <Field label="Setor responsável *"><Select value={form.setorId} onValueChange={v=>upd('setorId',v)}><SelectTrigger><SelectValue placeholder="Selecione"/></SelectTrigger><SelectContent>{setores.filter(s=>s.status==='ativo').map(s=><SelectItem key={s.id} value={s.id}>{s.nome}</SelectItem>)}</SelectContent></Select></Field>
