...
export default function OrdensProducaoPage() {
  ...
  const columns = [
    ...
    { key: 'planejada', label: 'Planejado', cellClass: 'text-right' },
    { key: 'prioridade', label: 'Prioridade', render: r => <StatusBadge status={String(r.prioridade || 'normal').toLowerCase()} /> },
    { key: 'progresso', label: 'Progresso', render: r => <div className="min-w-32"><div className="text-xs mb-1">{r.produzida || 0}/{r.planejada || 0}</div><Progress value={Number(r.progresso || 0)} className="h-1.5" /></div> },
