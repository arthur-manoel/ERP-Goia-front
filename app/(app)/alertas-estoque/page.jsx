          ...
          <Card key={p.id} className={`${Number(p.disponivel) === 0 ? 'border-red-500/40 bg-red-500/5' : 'border-amber-500/40 bg-amber-500/5'}`}>
            <CardContent className="p-5">
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                <div><div className="text-[10px] text-muted-foreground uppercase">Atual</div><div className={`text-lg font-semibold ${Number(p.disponivel) === 0 ? 'text-red-400' : 'text-amber-400'}`}>{Number(p.disponivel || 0).toLocaleString('pt-BR', { maximumFractionDigits: 2 })}</div></div>
