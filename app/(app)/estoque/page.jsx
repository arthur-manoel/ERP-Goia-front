...
export default function EstoquePage() {
  ...
  const columns = [
    ...
    { key: 'disponivel', label: 'Disponível', cellClass: 'text-right', render: row => <span className={`flex justify-end items-center gap-1 font-medium ${Number(row.disponivel) === 0 ? 'text-red-400' : Number(row.disponivel) <= Number(row.minimo) ? 'text-amber-400' : ''}`}>{Number(row.disponivel) <= Number(row.minimo) && <TooltipProvider delayDuration={200}><Tooltip><TooltipTrigger asChild><span tabIndex={0} aria-label={Number(row.disponivel) === 0 ? 'Produto sem estoque' : 'Estoque no mínimo ou abaixo'} className="inline-flex cursor-help rounded-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"><AlertTriangle className="h-3.5 w-3.5"/></span></TooltipTrigger><TooltipContent><p>{Number(row.disponivel) === 0 ? 'Produto sem estoque' : 'Estoque no mínimo ou abaixo'}</p><p className="text-xs opacity-80">Disponível: {qtd(row.disponivel)} · mínimo: {qtd(row.minimo)}</p></TooltipContent></Tooltip></TooltipProvider>}{qtd(row.disponivel)}</span> },
  ...
  return <div><PageHeader title="Estoque" description="Gerencie locais, visualize todos os itens e transfira saldos entre estoques." icon={Boxes}>
    ...
    <TabsContent value="itens" className="space-y-3"><div className="flex flex-wrap items-center gap-2 rounded-lg border border-border bg-card p-3"><Warehouse className="h-4 w-4 text-muted-foreground"/><span className="text-sm font-medium">Estoque:</span><Select value={estoqueSelecionado} onValueChange={setEstoqueSelecionado}><SelectTrigger className="w-56"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="__all__">Todos os estoques</SelectItem>{locais.data.map(local=><SelectItem key={local.id} value={local.id}>{local.nome}</SelectItem>)}</SelectContent></Select><span className="text-sm font-medium">Tipo:</span><Select value={tipoSelecionado} onValueChange={setTipoSelecionado}><SelectTrigger className="w-52"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="__all__">Todos os produtos</SelectItem><SelectItem value="materia_prima">Matéria-prima</SelectItem><SelectItem value="produto_acabado">Produto acabado</SelectItem></SelectContent></Select></div>
